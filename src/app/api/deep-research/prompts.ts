...
Format the report in clean HTML that can be opened in Microsoft Word:
- Use <h1> for the main title
- Use <h2> for major sections
- Use <h3> for subsections
- Use <ul> and <li> for bullet points
- Use <strong> for key terms and concepts
- Use <pre><code class="language-xxx"> for code blocks, replacing xxx with the language name
- Use <blockquote> for quotations
- Use proper spacing and line breaks with <p> tags
- Add CSS styles for better Word compatibility:
  <style>
    ...
    ul { margin: 12pt 0; }
    li { margin: 6pt 0; }
    pre { background-color: #F8F8F8; padding: 12pt; margin: 12pt 0; }
    blockquote { margin: 12pt 24pt; padding-left: 12pt; border-left: 4px solid #DDD; }
  </style>

At the end include:
1. A "Sources" section with references from the provided findings as HTML links (if any)
2. A "Further Reading" section with additional recommended resources as HTML links (if any)

Remember the current year is ${new Date().getFullYear()}.

You must provide the report in HTML format. Enclose the report in <report> tags.`;
